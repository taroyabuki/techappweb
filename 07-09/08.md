...
> [!NOTE]
> RDBMS（SQLiteとMySQL）をインストールして，Apacheを再起動，一般クエリログを有効にして，mysqlユーザのホームディレクトリを/var/lib/mysqlに変更して，MySQLを起動する。
...
> [!CAUTION]
> http://localhost/date-server.html の代わりにhttp://127.0.0.1/date-server.html とすると動作しない。アクセスするサーバの実体は同じだがオリジンが異なり，JavaScirptでの通信がSOP（Same-Origin Policy）に反するからである。date.phpに`header("Access-Control-Allow-Origin: http://127.0.0.1");`を追加すると，http://127.0.0.1/date-server.html でも動作するようになる。
