...
[session.php](html/session.php)では，クエリの`username`の値をサーバサイドのセッションに記録している。2のように同じブラウザでアクセスして，そのセッションを利用できる。
...
4. 被害者がそのサイト上でhttp://localhost/rename.php などの活動をする。この活動は，攻撃者が用意したセッションで行われる。
...
> [!TIP]
> この実験をサポートするスクリプトを用意している。次のようにcreate-session.shをシェルで実行して，1行目の2行目のURLをブラウザで開き，3行目をシェルで実行する。

```bash
# sh /var/www/techappweb/07-09/create-session.sh
http://localhost/hello.php?username=%3Cscript%3Edocument.cookie%3D'PHPSESSID%3D●●●'%3B%3C%2Fscript%3E
...
SNS-B|ホストB（localhost）|[session-none.php](html/session-none.php), [rename.php](html/rename.php), [session.php](html/session.php)
